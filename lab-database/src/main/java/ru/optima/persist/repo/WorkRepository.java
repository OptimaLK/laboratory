...
public interface WorkRepository extends JpaRepository<Work, Long> {
    ...
    @Query( "select distinct w from Work w join w.users u where w.workStatus.name = ?2 and u.id = ?1")
    List<Work> findAllWorksByUserIdWithStatusName(Long id, String statusName);

    @Query( "select distinct w from Work w join w.users u where w.workStatus.name = ?3 OR w.workStatus.name = ?2 and u.id = ?1")
